...
> **🚀 旅遊網站對照：互動狀態 (States)**
> 
> 在旅遊網站中，我們常用偽類來增強按鈕的點擊感。以下是傳統 CSS 與 Tailwind 的實現對照。
> 
> **傳統 CSS:**
> ```css
> .btn-book {
...
> **🚀 旅遊網站對照：尺寸單位 (Units)**
> 
> 在旅遊網站的 Hero Section 中，我們常使用 `rem` 來確保標題與間距在不同裝置上的比例協調。
> 
> **傳統 CSS:**
> ```css
> .hero-section {
>   padding: 3rem;       /* 相當於 48px */
>   text-align: center;
> }
> .hero-title {
>   font-size: 2.25rem;  /* 相當於 36px */
>   margin-bottom: 1rem; /* 相當於 16px */
> }
> ```
> 
> **Tailwind CSS:**
...
>     <h1 class="text-4xl mb-4">探索未知的世界</h1>
> </section>
> ```
> *(註：Tailwind 的 `p-12` 代表 $12 \times 0.25rem = 3rem$)*
---
...
- **容器屬性 (Container)：**
    ...
    - `flex-direction`：控制主軸方向 (`row`橫向, `column`縱向)。
    - `justify-content`：主軸對齊 (`center`, `space-between`, `space-around`)。
    - `align-items`：交錯軸對齊 (`center`, `start`, `end`)。
...
> [!TIP]
> **🚀 旅遊網站對照：佈局與間距 (Flex/Grid)**
> 
> 旅遊網站展示多個目的地時，使用 Flexbox 或 Grid 可以輕鬆達成。
> 
> **傳統 CSS (Flexbox):**
> ```css
> .gallery {
...
1. **B** (`padding` 設定內邊距，`margin` 是外邊距)
2. **C** ($100 + 10 \times 2 + 5 \times 2 = 130$)
3. **A** (`justify-content` 控制主軸對齊)
4. **B** (`border-box` 將內距與邊框包進寬度內)
5. **C** (`flex-wrap: wrap` 讓項目換行)
...
**文字演進：**
- **字體與大小**：使用 `font-family`, `font-size`, `font-weight`。
- **排版優化**：透過 `line-height` 與 `letter-spacing` 增加舒適度。
- **自動換行 (`word-wrap`)**：控制長單字是否強制換行。
- **多欄布局 (`columns`)**：將內容自動分成多欄（如同報紙）。
...
**裝飾技巧：**
- **背景樣式 (`background`)**：包含 `background-color` 與可縮放的 `background-image`。
- **邊框 (`border`)**：定義實線、虛線及圓角。
- **陰影效果 (`shadow`)**：`box-shadow` 用於區塊，`text-shadow` 用於文字。
...
> [!TIP]
> **🚀 旅遊網站對照：裝飾與特效 (Effects)**
> 
> 在旅遊卡片中，圓角、陰影與過渡動畫是提升精緻感的關鍵。
> 
> **傳統 CSS:**
> ```css
> .travel-card {
...
> **Tailwind CSS:**
> ```html
> <div class="rounded-2xl shadow-xl hover:shadow-2xl transition-shadow">
>     <!-- 卡片內容 -->
> </div>
> ```
...
**動態效果：**
- **變形 (`transform`)**：旋轉 `rotate`, 縮放 `scale`, 平移 `translate`。
- **過渡 (`transition`)**：屬性變化的平滑補間動畫。
- **動畫 (`animation`)**：搭配 `@keyframes` 定義複雜的循環動作。
...
1. **B** (`background-color` 設定背景顏色)
2. **D** (`transition` 是包含屬性、時間、速度曲線的縮寫)
3. **C** (`@keyframes` 定義動畫過程中的狀態)
4. **B** (`text-shadow` 處理文字陰影)
5. **A** (`border-box` 寬度 = 內容 + 內距 + 邊框)
...
*   **基本用法**：
    - 固定大小：`https://picsum.photos/200`
    - 指定寬高：`https://picsum.photos/300/200`
    - 隨機圖片：`https://picsum.photos/200?random=1`
*   **進階技巧 (補充)**：
    - **灰階效果**：`https://picsum.photos/200?grayscale` (適合復古或特定設計風格)
...
*   **VS Code 手動輸入**：
    - `lorem`：按 Tab/Enter 會產生一段標準長度的文字。
    - `lorem10`：產生 10 個英文字。
*   **Emmet 進階應用 (補充)**：
    - **產生多個段落**：輸入 `p*3>lorem10` 並按 Tab。這會產生 3 個 `<p>` 標籤，每個標籤內含 10 個假文字。
*   **中文字產生器 (ctlorem)**：
    - 需要安裝插件，輸入 `ctlorem50` 可產生 50 個中文字，這對於中文字型排版測試非常重要。
