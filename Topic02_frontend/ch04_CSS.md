...
3. **A** (`justify-content` 處理主軸對齊)
4. **B** (`border-box` 將內距與邊框包進寬度內)
5. **B** (`content-box` 寬度僅代表內容本身)
...
3. 在 Flexbox 容器中，`justify-content: center` 會如何影響元素？
   - A. 垂直置中
   - B. 讓元素變成圓形
   - C. 水平方向（主軸）對齊容器中心
   - D. 拉伸元素填滿空間

4. 如果想產生 50 個中文字，使用 `ctlorem` 插件應輸入什麼關鍵字？
   - A. `lorem50`
   - B. `ctlorem50`
   - C. `chineselorem50`
   - D. `text50`
