...
1️⃣ 在 Django 中，要建立一個名為 `members` 的子系統（應用程式 App），應該使用哪一個指令？
...
2️⃣ 關於 Django 的 MVT 設計模式，下列敘述何者**錯誤**？
...
3️⃣ 【簡答題】在 Django 專案中，當我們修改了 App 中的 `models.py` 後，必須執行哪兩個步驟才能將變更正式套用到資料庫中？請簡述這兩個步驟的指令與目的。
